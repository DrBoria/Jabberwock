import type { ToolTrace, TaskTrace } from "../types.js"
...
export class Tracer {
	...
	constructor(private logFn: (msg: string, level?: string) => void) {}
		...
		try {
			...
			const redact = (obj: Record<string, unknown>) => {
				for (const key in obj) {
					...
					} else if (obj[key] && typeof obj[key] === "object") redact(obj[key] as Record<string, unknown>)
				}
			}
			redact(sanitized)
			return sanitized
	...
	private detectCycle(taskId: string, toolName: string, params: unknown) {
