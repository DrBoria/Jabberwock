...
import type { MessageInterceptor } from "../utils/interceptor.js"
...
export function registerEventBusTools(
	...
	mcpServer.tool(
		...
		async ({ direction, type, action, response }) => {
			try {
				interceptor.set({ direction: direction as "send" | "receive", type, action, response })
				return {
					content: [
						{
							type: "text",
	...
	mcpServer.tool("get_active_interceptors", {}, async () => {
		try {
			const interceptors = interceptor.getActive()
			return {
				content: [
					{
						type: "text",
