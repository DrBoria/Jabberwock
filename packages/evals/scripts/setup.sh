...
pnpm --filter @jabberwock/evals db:push --force &>/dev/null || exit 1
echo "✅ Done"
...
if ! nc -z localhost 3446; then
  ...
  if [[ "$start_evals" =~ ^[Yy]|^$ ]]; then
    pnpm --filter @jabberwock/web-evals dev
  else
    echo "💡 You can start it anytime with 'pnpm --filter @jabberwock/web-evals dev'."
  fi
else
