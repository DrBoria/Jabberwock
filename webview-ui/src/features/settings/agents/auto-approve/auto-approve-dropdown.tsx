...
import { ToggleSwitch } from "@src/features/foundation/ui/toggle-switch"
import { Button } from "@src/features/foundation/ui/button"

import { AutoApproveSetting, autoApproveSettingsConfig } from "@src/features/settings/components/AutoApproveToggle"
...
export const AutoApproveDropdown = observer(({ disabled = false, triggerClassName = "" }: AutoApproveDropdownProps) => {
	...
	const setAutoApprovalEnabled = React.useCallback((value: boolean) => rootStore.setAutoApprovalEnabled(value), [])
	const setAlwaysAllowReadOnly = React.useCallback((value: boolean) => rootStore.setAlwaysAllowReadOnly(value), [])
	const setAlwaysAllowWrite = React.useCallback((value: boolean) => rootStore.setAlwaysAllowWrite(value), [])
	const setAlwaysAllowExecute = React.useCallback((value: boolean) => rootStore.setAlwaysAllowExecute(value), [])
	const setAlwaysAllowMcp = React.useCallback((value: boolean) => rootStore.setAlwaysAllowMcp(value), [])
	const setAlwaysAllowModeSwitch = React.useCallback(
		(value: boolean) => rootStore.setAlwaysAllowModeSwitch(value),
		[],
	)
	const setAlwaysAllowSubtasks = React.useCallback((value: boolean) => rootStore.setAlwaysAllowSubtasks(value), [])
	const setAlwaysAllowFollowupQuestions = React.useCallback(
		(value: boolean) => rootStore.setAlwaysAllowFollowupQuestions(value),
		[],
	)
	...
	const tooltipText = React.useMemo(() => {
		if (!effectiveAutoApprovalEnabled) {
			return t("chat:autoApprove.triggerLabelOff")
		}
		if (enabledCount === totalCount) {
			return t("chat:autoApprove.triggerLabelAll")
		}
		return t("chat:autoApprove.triggerLabel", { count: enabledCount })
	}, [effectiveAutoApprovalEnabled, enabledCount, totalCount, t])
