...
export const CodeIndexPopover: React.FC<CodeIndexPopoverProps> = observer(
	({ children, indexingStatus: externalIndexingStatus }) => {
		...
		const codebaseIndexConfig = s.codebaseIndexConfig
		const codebaseIndexModels = s.codebaseIndexModels
		const cwd = s.cwd
		...
		const updateSetting = (key: keyof LocalCodeIndexSettings, value: unknown) => {
							...
							{currentSettings.codebaseIndexEnabled && (
								<div className="flex items-center gap-2 pt-4 pb-1">
									<input
										...
										onChange={(e) =>
											rootStore.settings.setAutoEnableDefault(
												(e.target as HTMLInputElement).checked,
											)
										}
							...
							{currentSettings.codebaseIndexEnabled && (
								<div className="flex items-center gap-2 pt-1 pb-2">
									<input
										...
										onChange={(e) =>
											rootStore.settings.toggleWorkspaceIndexing(
												(e.target as HTMLInputElement).checked,
											)
										}
										className="accent-vscode-focusBorder"
								...
								<div className="flex gap-2">
									...
									{currentSettings.codebaseIndexEnabled &&
										indexingStatus.systemStatus === "Indexing" && (
											<Button
												variant="destructive"
												onClick={() => rootStore.settings.stopIndexing()}>
											...
											<AlertDialog>
												...
												<AlertDialogContent>
													...
													<AlertDialogFooter>
														...
														<AlertDialogAction
															onClick={() => rootStore.settings.clearIndexData()}>
