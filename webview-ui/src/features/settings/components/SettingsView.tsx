...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@src/features/foundation/ui/tooltip"
import { StandardTooltip } from "@src/features/foundation/ui/standard-tooltip"

import { Tab, TabContent, TabHeader, TabList, TabTrigger } from "@src/features/foundation/components/Tab"
...
import { WorktreesView } from "@src/features/settings/components/WorktreesView"
...
const SettingsView = forwardRef<SettingsViewRef, SettingsViewProps>(({ onDone, targetSection }, ref) => {
	...
	}, [currentApiConfigName, s.apiConfiguration]) // apiConfiguration added to detect in-place config updates

	// Bust the cache when settings are imported.
	useEffect(() => {
		if (settingsImportedAt && prevSettingsImportedAt.current !== settingsImportedAt) {
			setCachedState((prevCachedState) => ({ ...prevCachedState, ...s }))
			setChangeDetected(false)
			prevSettingsImportedAt.current = settingsImportedAt
		}
	}, [settingsImportedAt, s])

	const setCachedStateField: SetCachedStateField<keyof ExtensionState> = useCallback((field, value) => {
	...
	const onConfirmDialogResult = useCallback(
		(confirm: boolean) => {
			if (confirm) {
				...
				setCachedState(s) // Revert to original state
				setChangeDetected(false) // Reset change flag
				confirmDialogHandler.current?.() // Execute the pending action (e.g., tab switch)
			}
			// If confirm is false (Cancel), do nothing, dialog closes automatically
		},
		[s], // Depend on s to get the latest original state
	)
