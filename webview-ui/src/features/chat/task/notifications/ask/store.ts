 ...
 *   2. Sets button state (currentAsk, enableButtons, primaryButtonText, etc.)
...
function computeIsStreaming(
	...
	const lastApiReqStarted = findLast(modifiedMessages, (message: Notification) => message.say === "api_req_started")
	...
	const isToolCurrentlyAsking =
		isLastAsk &&
		currentAsk !== "" &&
		((enableButtons && primaryButtonText !== "") || currentAsk === "interactive_app")
 ...
 */
function computeModifiedMessages(messages: Notification[]): Notification[] {
...
export const AskStore = types.model("AskStore", {}).actions((self) => {
	...
	function handleSayMessage(lastMessage: Notification): void {
		const ui = getUI()
	...
	return {
		...
		processAskMessage(
			...
			ui.setIsFollowUpAutoApprovalPaused(
				!!(inputValue && inputValue.trim().length > 0 && ui.currentAsk === "followup"),
			)
		 ...
		 * Called separately from processAskMessage because it depends on currentAsk
