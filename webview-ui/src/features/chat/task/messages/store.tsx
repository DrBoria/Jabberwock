...
export const Message = types.model("Message", {
	id: types.identifier,
	role: types.string, // "user", "assistant"
	content: types.frozen(), // For Anthropic-style multi-block content
	type: types.string, // "say" or "ask"
	say: types.string, // "text", "error", etc.
	ask: types.string, // "tool", "followup", etc.
 ...
 */
export function createMessagesListActions(self: { clearInput(): void; setSendingDisabled(val: boolean): void }) {
	return {
		// ── Ask response ───────────────────────────────────────────
