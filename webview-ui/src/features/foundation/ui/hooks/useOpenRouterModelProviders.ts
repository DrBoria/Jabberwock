...
async function getOpenRouterProvidersForModel(modelId: string, baseUrl?: string) {
	...
	try {
		...
		if (!result.success) {
			console.error("[jabberwock] OpenRouter API response validation failed:", result.error)
			return models
		}
		...
		if (error instanceof z.ZodError) {
			console.error(`[jabberwock] OpenRouter API response validation failed:`, error.errors)
		} else {
			console.error(`[jabberwock] Error fetching OpenRouter providers:`, error)
		}
	}
