	...
	useTranslation: () => ({
		t: (key: string) => {
			const map: Record<string, string> = {
				"chat:fileOperations.wantsToEdit": "Jabberwock wants to edit this file",
				"chat:fileOperations.wantsToEditProtected": "Jabberwock wants to edit a protected file",
				"chat:fileOperations.wantsToEditOutsideWorkspace": "Jabberwock wants to edit outside workspace",
				"chat:fileOperations.wantsToApplyBatchChanges": "Jabberwock wants to apply batch changes",
			}
			return map[key] || key
		},
	}),
...
describe("ChatRow - inline diff stats and actions", () => {
	...
	it("preserves protected and outside-workspace messaging in unified branch", () => {
		...
		expect(screen.getByText("Jabberwock wants to edit outside workspace")).toBeInTheDocument()
		...
		expect(screen.getByText("Jabberwock wants to edit a protected file")).toBeInTheDocument()
	...
	it("keeps batch diff handling for unified edit tools", () => {
		...
		expect(screen.getByText("Jabberwock wants to apply batch changes")).toBeInTheDocument()
