...
function extractTextFromItem(item: vscode.LanguageModelChatMessage["content"][number]): string {
	let text = ""
...
export function extractTextCountFromMessage(message: vscode.LanguageModelChatMessage): string {
	...
	return message.content.reduce((text, item) => text + extractTextFromItem(item), "")
}
