...
/**
 * Parses colon-separated timestamp formats: "mm:ss" or "hh:mm:ss"
 */
function parseColonTimestamp(timestamp: string): number | null {
	...
	if (colonMatch[3]) {
		const c = parseInt(colonMatch[3], 10)
...
/**
 * Parses h/m/s format timestamp like "1h2m3s", "2m30s", "45s"
 */
function parseHmsTimestamp(timestamp: string): number {
	let totalSeconds = 0
...
function parseTimestampToSeconds(timestamp: string): number {
	...
	const colonResult = parseColonTimestamp(timestamp)
	if (colonResult !== null) {
		return colonResult
	}

	return parseHmsTimestamp(timestamp)
}
...
export function isYouTubeUrl(url: string): boolean {
	const isMatch = YOUTUBE_URL_REGEX.test(url)
	return isMatch
}
