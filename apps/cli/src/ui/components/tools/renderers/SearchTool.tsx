...
export function SearchTool({ toolData }: ToolRendererProps) {
	...
	const regex = getSafeString(toolData.regex)
	const query = getSafeString(toolData.query)
	const filePattern = getSafeString(toolData.filePattern)
	const path = getSafeString(toolData.path)
