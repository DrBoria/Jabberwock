...
suite.skip("Jabberwock use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		...
		// Create .jabberwock directory and MCP configuration file
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(JabberwockEventName.TaskStarted, taskStartedHandler)
		...
		api.on(JabberwockEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for Jabberwock to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this Jabberwock won't load the MCP server
		...
		try {
			...
			api.off(JabberwockEventName.TaskStarted, taskStartedHandler)
			api.off(JabberwockEventName.TaskCompleted, taskCompletedHandler)
		}
	})
	...
	test("Should request MCP filesystem list_directory tool and complete successfully", async function () {
		...
		try {
			...
			const hasRooDir = responseText.includes(".jabberwock")

			// At least one of our test files or the .jabberwock directory should be present
			assert.ok(
				hasTestFile || hasDataFile || hasRooDir,
				`MCP server response should contain our test files or .jabberwock directory. Expected to find: '${path.basename(testFiles.simple)}', '${path.basename(testFiles.testData)}', or '.jabberwock'. Got: ${responseText.substring(0, 200)}...`,
			)
	...
	test.skip("Should request MCP filesystem directory_tree tool and complete successfully", async function () {
		...
		try {
			...
			const hasTestFiles =
				...
				responseText.includes(".jabberwock") ||
				responseText.includes(".txt") ||
				responseText.includes(".json") ||
	...
	test.skip("Should validate MCP request message format and complete successfully", async function () {
		...
		const messageHandler = ({ message }: { message: Notification }) => {
			...
			if (message.type === "ask" && message.ask === "use_mcp_server") {
				...
				// Validate the message format matches McpServerRequestData interface
				if (message.text) {
					try {
						const mcpRequest = JSON.parse(message.text)
						mcpToolName = mcpRequest.toolName
