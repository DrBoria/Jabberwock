...
describe("autoImportSettings", () => {
	...
	it("should skip auto-import when settings file does not exist", async () => {
		const settingsPath = "~/Documents/jabberwock-config.json"
		vi.mocked(vscode.workspace.getConfiguration).mockReturnValue({
		...
		expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
			"[AutoImport] Checking for settings file at: /home/user/Documents/jabberwock-config.json",
		)
		expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
			"[AutoImport] Settings file not found at /home/user/Documents/jabberwock-config.json, skipping auto-import",
		)
		expect(mockProviderSettingsManager.import).not.toHaveBeenCalled()
	})
