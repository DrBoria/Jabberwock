// npx vitest run core/webview/__tests__/EventBridge.sticky-profile.spec.ts
...
describe("EventBridge - Sticky Provider Profile", () => {
	...
	describe("activateProviderProfile", () => {
		...
		it("should save provider profile to task metadata when switching profiles", async () => {
			...
			await (provider as any).taskHistoryStore.upsert({
				id: mockTask.taskId,
				ts: Date.now(),
				task: "Test task",
				number: 1,
			...
			await (provider as any).activateProviderProfile({ name: "new-profile" })
	...
	describe("createTaskWithHistoryItem", () => {
		it("should restore provider profile from history item when reopening task outside CLI runtime", async () => {
			...
			const activateProviderProfileSpy = vi
				.spyOn(provider as any, "activateProviderProfile")
		...
		it("should use current profile if history item has no saved apiConfigName", async () => {
			...
			const callsForApiConfigName = activateProviderProfileSpy.mock.calls.filter(
				(call) => call[0] && "name" in (call[0] as any) && (call[0] as any).name === historyItem.apiConfigName,
			)
		...
		it("should override mode-based config with task's apiConfigName", async () => {
			...
			vi.spyOn(provider as any, "activateProviderProfile").mockImplementation(async (args: any) => {
				if ("name" in args) {
	...
	describe("Multiple workspaces isolation", () => {
		it("should preserve task profile when switching profiles in another workspace", async () => {
			...
			for (const item of taskHistory) {
				await (provider as any).taskHistoryStore.upsert(item as any)
			}

			// Mock updateTaskHistory
			vi.spyOn(provider as any, "updateTaskHistory").mockImplementation((item: any) => {
			...
			await (provider as any).activateProviderProfile({ name: "profile-c" })
	...
	describe("Error handling", () => {
		it("should handle errors gracefully when saving profile fails", async () => {
			...
			vi.spyOn(provider as any, "updateTaskHistory").mockRejectedValue(new Error("Save failed"))
			...
			await expect((provider as any).activateProviderProfile({ name: "new-profile" })).resolves.not.toThrow()
	...
	describe("Profile restoration with activateProfile failure", () => {
		it("should continue task restoration even if activateProviderProfile fails", async () => {
			...
			vi.spyOn(provider as any, "activateProviderProfile").mockRejectedValue(new Error("Activation failed"))
