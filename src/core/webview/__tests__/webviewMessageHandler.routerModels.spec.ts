...
describe("webviewMessageHandler - requestRouterModels provider filter", () => {
	...
	beforeEach(() => {
		...
		getModelsMock.mockImplementation(async (options: any) => {
			switch (options?.provider) {
				case "jabberwock":
					return { "jabberwock/sonnet": { contextWindow: 8192, supportsPromptCache: false } }
				case "openrouter":
	...
	it("fetches only requested provider when values.provider is present ('jabberwock')", async () => {
		...
		// Only "jabberwock" key should be present
		const keys = Object.keys(routerModels)
		expect(keys).toEqual(["jabberwock"])
		expect(Object.keys(routerModels.jabberwock || {})).toContain("jabberwock/sonnet")

		// getModels should have been called exactly once for jabberwock
		const providersCalled = getModelsMock.mock.calls.map((c: any[]) => c[0]?.provider)
		expect(providersCalled).toEqual(["jabberwock"])
	})
	...
	it("defaults to aggregate fetching when no provider filter is sent", async () => {
		...
		expect(routerModels).toHaveProperty("jabberwock")
