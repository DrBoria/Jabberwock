// npx vitest core/webview/__tests__/EventBridge.sticky-mode.spec.ts
...
import type { HistoryItem, ProviderName } from "@jabberwock/types"
...
describe("EventBridge - Sticky Mode", () => {
	...
	describe("handleModeSwitch", () => {
		...
		it("should save mode to task metadata when switching modes", async () => {
			...
			const updateTaskHistorySpy = vi
				.spyOn(provider as any, "updateTaskHistory")
				.mockImplementation(() => Promise.resolve([]))

			// Add task to provider stack
			await (provider as any).addClineToStack(mockTask)

			// Switch mode
			await (provider as any).handleModeSwitch("architect")
		...
		it("should update task's taskMode property when switching modes", async () => {
			...
			// Add task to provider stack
			await (provider as any).addClineToStack(mockTask as any)
			...
			vi.spyOn(provider as any, "updateTaskHistory").mockImplementation(() => Promise.resolve([]))
	...
	describe("createTaskWithHistoryItem", () => {
		it("should restore mode from history item when reopening task", async () => {
			...
			await (provider as any).createTaskWithHistoryItem(historyItem)
		...
		it("should use current mode if history item has no saved mode", async () => {
			...
			const handleModeSwitchSpy = vi.spyOn(provider as any, "handleModeSwitch").mockResolvedValue(undefined)
	...
	describe("Task metadata persistence", () => {
		it("should include mode in task metadata when creating history items", async () => {
			...
			vi.spyOn(provider as any, "updateTaskHistory").mockImplementation((item) => {
			...
			await (provider as any).handleModeSwitch("debug")
	...
	describe("Integration with new_task tool", () => {
		it("should preserve parent task mode when creating subtasks", async () => {
			...
			const updateTaskHistoryMock = vi.spyOn(provider as any, "updateTaskHistory")
			updateTaskHistoryMock.mockImplementation((item: any) => {
			...
			await (provider as any).addClineToStack(parentTask)
			...
			await (provider as any).addClineToStack(subtask)
			...
			await (provider as any).handleModeSwitch("code")
	...
	describe("Error handling", () => {
		it("should handle errors gracefully when saving mode fails", async () => {
			...
			await expect((provider as any).handleModeSwitch("architect")).resolves.not.toThrow()
		...
		it("should handle null/undefined mode gracefully", async () => {
			...
			await expect((provider as any).createTaskWithHistoryItem(historyItem)).resolves.not.toThrow()
		...
		it("should restore API configuration when restoring task from history with mode", async () => {
			...
			// Save API configs
			await (provider as any).upsertProviderProfile("code-config", codeApiConfig)
			await (provider as any).upsertProviderProfile("architect-config", architectApiConfig)

			// Get the config IDs
			const codeConfigId = (provider as any).getProviderProfileEntry("code-config")?.id
			const architectConfigId = (provider as any).getProviderProfileEntry("architect-config")?.id
	...
	describe("Concurrent mode switches", () => {
		it("should handle concurrent mode switches on the same task", async () => {
			...
			const switches = [
				(provider as any).handleModeSwitch("architect"),
				(provider as any).handleModeSwitch("debug"),
				(provider as any).handleModeSwitch("code"),
			]
	...
	describe("Mode switch failure scenarios", () => {
		it("should handle invalid mode gracefully", async () => {
			...
			await (provider as any).handleModeSwitch("invalid-mode" as any)
		...
		it("should handle errors during mode switch gracefully", async () => {
			...
			await expect((provider as any).handleModeSwitch("architect")).rejects.toThrow("Emit failed")
		...
		it("should handle updateTaskHistory failures", async () => {
			...
			vi.spyOn(provider as any, "updateTaskHistory").mockRejectedValue(new Error("Update failed"))
			...
			await expect((provider as any).handleModeSwitch("architect")).rejects.toThrow("Update failed")
	...
	describe("Multiple tasks switching modes simultaneously", () => {
		it("should handle multiple tasks switching modes independently", async () => {
			...
			await (provider as any).addClineToStack(task1 as any)
			await (provider as any).addClineToStack(task2 as any)
			await (provider as any).addClineToStack(task3 as any)
			...
			const switch1 = (provider as any).handleModeSwitch("architect")

			getCurrentTaskSpy.mockReturnValue(task2 as any)
			const switch2 = (provider as any).handleModeSwitch("debug")

			getCurrentTaskSpy.mockReturnValue(task3 as any)
			const switch3 = (provider as any).handleModeSwitch("code")
	...
	describe("Task initialization timing edge cases", () => {
		it("should handle mode restoration during slow task initialization", async () => {
			...
			vi.spyOn(provider as any, "getTaskWithId").mockImplementation(async () => {
				await new Promise((resolve) => setTimeout(resolve, 100))
				return {
					historyItem,
			...
			const initPromise = (provider as any).createTaskWithHistoryItem(historyItem)
		...
		it("should handle rapid task switches during mode changes", async () => {
			...
			for (const task of tasks) {
				await (provider as any).addClineToStack(task as any)
			}
			...
			tasks.forEach((task, index) => {
				...
				switches.push((provider as any).handleModeSwitch(mode as any))
			})
