// pnpm --filter jabberwock test core/webview/__tests__/EventBridge.spec.ts
...
import { Task, TaskOptions } from "../../../features/chat/task/Task"
...
describe("EventBridge", () => {
	...
	beforeEach(() => {
		...
		updateGlobalStateSpy = vi.spyOn((provider as any).contextProxy, "setValue")
	...
	test("dispose is idempotent — second call is a no-op", async () => {
		...
		// dispose body runs only once: log "Disposing EventBridge..." appears once
		const disposeCalls = (mockOutputChannel.appendLine as ReturnType<typeof vi.fn>).mock.calls.filter(
			([msg]) => typeof msg === "string" && msg.includes("Disposing EventBridge..."),
		)
	...
	test("clearTask aborts current task", async () => {
		...
		await (provider as any).addClineToStack(mockCline)

		// get the stack size before the abort call
		const stackSizeBeforeAbort = (provider as any).getTaskStackSize()
		...
		const stackSizeAfterAbort = (provider as any).getTaskStackSize()
	...
	describe("clearTask message handler", () => {
		...
		test("calls clearTask (delegation handled via metadata)", async () => {
			...
			const clearTaskSpy = vi.spyOn(provider as any, "clearTask").mockResolvedValue(undefined)
			const postStateToWebviewSpy = vi.spyOn(provider as any, "postStateToWebview").mockResolvedValue(undefined)
		...
		test("calls clearTask even with parent task (delegation via metadata)", async () => {
			...
			await (provider as any).addClineToStack(childTask)
		...
		test("correctly identifies task scenario for issue #4602", async () => {
			...
			expect((provider as any).getTaskStackSize()).toBe(1)
	...
	test("addClineToStack adds multiple Cline instances to the stack", async () => {
		...
		await (provider as any).addClineToStack(mockCline1)
		await (provider as any).addClineToStack(mockCline2)

		// verify cline instances were added to the stack
		expect((provider as any).getTaskStackSize()).toBe(2)
	...
	describe("deleteMessage", () => {
		...
		test("handles deletion with confirmation dialog", async () => {
			...
			await (provider as any).addClineToStack(mockCline) // Add the mocked instance to the stack
...
describe.skip("ContextProxy integration", () => {
	...
	beforeEach(() => {
		...
		provider = new EventBridge(mockContext, mockOutputChannel, "sidebar", mockContextProxy)
	})
...
describe("getTelemetryProperties", () => {
	...
	describe("cloud authentication telemetry", () => {
		...
		test("includes cloud authentication property when user is not authenticated", async () => {
			const { getCloudService } = await import("@jabberwock/cloud")
			vi.mocked(getCloudService().isAuthenticated).mockReturnValue(false)
		...
		test("handles CloudService errors gracefully", async () => {
			// Update the CloudService mock to throw an error
			const { getCloudService } = await import("@jabberwock/cloud")
			vi.mocked(getCloudService).mockImplementation(() => {
		...
		test("handles CloudService method errors gracefully", async () => {
			const { getCloudService } = await import("@jabberwock/cloud")
			vi.mocked(getCloudService().isAuthenticated).mockImplementation(() => {
...
describe("EventBridge - Router Models", () => {
	let provider: EventBridge
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
...
describe("EventBridge - Comprehensive Edit/Delete Edge Cases", () => {
	let provider: EventBridge
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
	...
	describe("getTaskWithId", () => {
		it("returns empty apiConversationHistory when file is missing", async () => {
			...
			const deleteTaskSpy = vi.spyOn(provider as any, "deleteTaskFromState")
