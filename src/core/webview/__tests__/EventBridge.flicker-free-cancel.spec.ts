...
vi.mock("@jabberwock/cloud", () => ({
	...
	getJabberwockApiUrl: vi.fn().mockReturnValue("https://api.jabberwock.com"),
}))
...
describe("EventBridge flicker-free cancel", () => {
	...
	beforeEach(() => {
		...
		provider = new EventBridge(mockContext, mockOutputChannel, "sidebar", mockContextProxy as any)
		...
		;(provider as any).performPreparationTasks = vi.fn().mockResolvedValue(undefined)(
			provider as any,
		).getTaskWithId = vi.fn().mockImplementation((id) =>
			Promise.resolve({
				historyItem: {
					id,
					number: 1,
	...
	it("should not remove current task from stack when rehydrating same taskId", async () => {
		...
		const removeClineFromStackSpy = vi.spyOn(provider as any, "removeClineFromStack")
	...
	it("should remove task from stack when creating different task", async () => {
		...
		const removeClineFromStackSpy = vi.spyOn(provider as any, "removeClineFromStack").mockResolvedValue(undefined)
