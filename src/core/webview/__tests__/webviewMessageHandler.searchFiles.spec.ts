...
describe("webviewMessageHandler - searchFiles with RooIgnore filtering", () => {
	...
	beforeEach(() => {
		...
		// Spy on the mock JabberwockIgnoreController prototype methods
		mockFilterPaths = vi.fn()
		mockDispose = vi.fn()

		// Override the filterPaths method on the prototype
		;(JabberwockIgnoreController.prototype as any).filterPaths = mockFilterPaths
		;(JabberwockIgnoreController.prototype as any).initialize = vi.fn().mockResolvedValue(undefined)
		;(JabberwockIgnoreController.prototype as any).dispose = mockDispose
	...
	it("should filter results using JabberwockIgnoreController when showJabberwockIgnoredFiles is false", async () => {
		...
		// Setup state with showJabberwockIgnoredFiles = false
		;(mockEventBridge.getState as Mock).mockResolvedValue({
		...
		;(mockEventBridge.getCurrentTask as Mock).mockReturnValue(null)
	...
	it("should not filter results when showJabberwockIgnoredFiles is true", async () => {
		// Setup mock results from file search
		const mockResults = [
			{ path: "src/index.ts", type: "file" as const, label: "index.ts" },
			{ path: "secrets/config.json", type: "file" as const, label: "config.json" },
		...
		// Setup state with showJabberwockIgnoredFiles = true
		;(mockEventBridge.getState as Mock).mockResolvedValue({
	...
	it("should use existing JabberwockIgnoreController from current task", async () => {
		// Setup mock results from file search
		const mockResults = [
			{ path: "src/index.ts", type: "file" as const, label: "index.ts" },
		...
		// Create a mock task with its own JabberwockIgnoreController
		const taskFilterPaths = vi.fn().mockReturnValue(["src/index.ts"])
		const taskJabberwockIgnoreController = {
			filterPaths: taskFilterPaths,
			initialize: vi.fn(),
		}
		;(mockEventBridge.getCurrentTask as Mock).mockReturnValue({
			taskId: "test-task-id",
			jabberwockIgnoreController: taskJabberwockIgnoreController,
		})
	...
	it("should default showJabberwockIgnoredFiles to false when state is null", async () => {
		// Setup mock results from file search
		const mockResults = [{ path: "src/index.ts", type: "file" as const, label: "index.ts" }]
		mockSearchWorkspaceFiles.mockResolvedValue(mockResults)

		// Setup state to return null
		;(mockEventBridge.getState as Mock).mockResolvedValue(null)
		...
		// Verify filterPaths was called (showJabberwockIgnoredFiles defaults to false)
		expect(mockFilterPaths).toHaveBeenCalled()
	})

	it("should dispose temporary JabberwockIgnoreController after use", async () => {
		// Setup mock results from file search
		const mockResults = [{ path: "src/index.ts", type: "file" as const, label: "index.ts" }]
		mockSearchWorkspaceFiles.mockResolvedValue(mockResults)
