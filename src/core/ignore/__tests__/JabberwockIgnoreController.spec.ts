// npx vitest core/ignore/__tests__/JabberwockIgnoreController.spec.ts

import type { Mock } from "vitest"

import { JabberwockIgnoreController, LOCK_TEXT_SYMBOL } from "../JabberwockIgnoreController"
...
describe("JabberwockIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: JabberwockIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new JabberwockIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .jabberwockignore exists
		 */
		it("should load .jabberwockignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .jabberwockignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".jabberwockignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".jabberwockignore"), "utf8")

			// Verify content was stored
			expect(controller.jabberwockIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .jabberwockignore doesn't exist
		 */
		it("should allow all access when .jabberwockignore doesn't exist", async () => {
			// Setup mocks to simulate missing .jabberwockignore file
			...
			expect(controller.jabberwockIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .jabberwockignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".jabberwockignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .jabberwockignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .jabberwockignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		beforeEach(async () => {
			// Setup .jabberwockignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**\n*.log")
			await controller.initialize()
		})
		...
		/**
		 * Tests the default behavior when no .jabberwockignore exists
		 */
		it("should allow all access when no .jabberwockignore content", async () => {
			// Create a new controller with no .jabberwockignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new JabberwockIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .jabberwockignore exists
		 */
		it("should allow all commands when no .jabberwockignore exists", async () => {
			// Create a new controller with no .jabberwockignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new JabberwockIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .jabberwockignore
		 */
		it("should generate formatted instructions when .jabberwockignore exists", async () => {
			// Setup .jabberwockignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .jabberwockignore exists
		 */
		it("should return undefined when no .jabberwockignore exists", async () => {
			// Setup no .jabberwockignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .jabberwockignore is created
		 */
		it("should reload .jabberwockignore when file is created", async () => {
			// Setup initial state without .jabberwockignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .jabberwockignore
			controller = new JabberwockIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .jabberwockignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .jabberwockignore is changed
		 */
		it("should reload .jabberwockignore when file is changed", async () => {
			// Setup initial state with .jabberwockignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .jabberwockignore is deleted
		 */
		it("should reset when .jabberwockignore is deleted", async () => {
			// Setup initial state with .jabberwockignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
