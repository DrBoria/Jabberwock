 ...
 */
function todoLog(msg: string, data?: Record<string, unknown>) {
	const logMsg = `[TODO-LOG] [UseMcpToolTool] ${msg}${data ? " " + JSON.stringify(data) : ""}`
	console.log(logMsg)
	try {
		const { diagnosticsManager } = require("../../devtools/DiagnosticsManager")
 ...
 */
export async function processDeterministicDelegation(task: Task, text: string): Promise<DelegationResult | null> {
	...
	for (const todoTask of approvedTasks) {
		...
		try {
			...
			console.error(`[DeterministicDelegation] Failed to delegate ${todoTask.id}: ${errMsg}`)
