...
describe("executeCommandTool", () => {
	...
	const originalCliRuntime = process.env.JABBERWOCK_CLI_RUNTIME
	...
	afterEach(() => {
		process.env.JABBERWOCK_CLI_RUNTIME = originalCliRuntime
	})
	...
	describe("Error handling", () => {
		...
		it("should handle rooignore validation failures", async () => {
			...
			mockCline.jabberwockIgnoreController = {
				validateCommand: validateCommandMock,
			}

			const mockRooIgnoreError = "RooIgnore error"
			;(formatResponse.jabberwockIgnoreError as any).mockReturnValue(mockRooIgnoreError)
			...
			expect(formatResponse.jabberwockIgnoreError).toHaveBeenCalledWith(".env")
