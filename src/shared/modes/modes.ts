...
import { TOOL_GROUPS, ALWAYS_AVAILABLE_TOOLS } from "@shared/tools/tools.groups"
 ...
 */
function buildModeSelection(mode: ModeConfig, promptComponent?: PromptComponent) {
	return {
		roleDefinition: promptComponent?.roleDefinition || mode.roleDefinition || "",
		baseInstructions: promptComponent?.customInstructions || mode.customInstructions || "",
...
export function getModeSelection(mode: string, promptComponent?: PromptComponent, customModes?: ModeConfig[]) {
	...
	if (customMode) {
		return buildModeSelection(customMode)
	}

	const builtInMode = findModeBySlug(mode, modes) || modes[0]
	return buildModeSelection(builtInMode, promptComponent)
}
