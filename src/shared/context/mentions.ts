...
/**
 * Replaces backslash-escaped spaces with regular spaces.
 * Used to unescape file paths that contain escaped spaces (e.g., `/path/to/file\ with\ spaces.txt`).
 */
export function unescapeSpaces(path: string): string {
	const unescaped = path.replaceAll("\\ ", " ")
	return unescaped
}
