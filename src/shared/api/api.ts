...
function isAnthropicContext(modelId: string, format?: string): boolean {
	return (
...
function clampMaxTokens(modelId: string, model: ModelInfo, format?: string): number | undefined {
	if (!model.maxTokens) {
		if (format) {
			return undefined
		}
...
export const getModelMaxOutputTokens = ({
	...
	const anthropicCtx = isAnthropicContext(modelId, format)

	if (model.supportsReasoningBudget && anthropicCtx) {
		return ANTHROPIC_DEFAULT_MAX_TOKENS
	}

	if (anthropicCtx && (!model.maxTokens || model.maxTokens === 0)) {
		return ANTHROPIC_DEFAULT_MAX_TOKENS
	}

	return clampMaxTokens(modelId, model, format)
}
