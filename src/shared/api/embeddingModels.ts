...
import type { EmbedderProvider, EmbeddingModelProfiles } from "@jabberwock/types"
 ...
 */
const DEFAULT_MODEL_IDS: Partial<Record<EmbedderProvider, string>> = {
	openai: "text-embedding-3-small",
	"openai-compatible": "text-embedding-3-small",
	gemini: "gemini-embedding-001",
	mistral: "codestral-embed-2505",
	"vercel-ai-gateway": "openai/text-embedding-3-large",
	bedrock: "amazon.titan-embed-text-v2:0",
	openrouter: "openai/text-embedding-3-large",
}

function getOllamaDefaultModelId(): string {
...
export function getDefaultModelId(provider: EmbedderProvider): string {
	if (provider === "ollama") {
		return getOllamaDefaultModelId()
	}
	...
	console.warn(`[jabberwock] Unknown provider for default model ID: ${provider}. Falling back to OpenAI default.`)
