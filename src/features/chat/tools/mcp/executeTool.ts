...
export async function executeToolAndProcessResult(
	...
	let interactiveAppMeta: { [key: string]: unknown } | undefined
	...
	if (toolResult) {
		if (toolResult._meta?.ui) {
			...
			// ask(task.taskId, ) is called from within presentAssistantMessage, which holds the lock.
					...
					try {
						...
						if (delegationResult === null) {
							...
							if (delegationResult.isDelegated) {
								...
								// so we must NOT call say(task.taskId, ) or pushToolResult() on the
	...
	await mcpBroadcast(task.taskId, "mcp_server_response", sayText, images)
