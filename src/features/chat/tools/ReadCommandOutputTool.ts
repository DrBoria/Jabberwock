...
import { systemBroadcast, agentBroadcast } from "../task/messages/actions/say"
...
export class ReadCommandOutputTool extends BaseTool<"read_command_output"> {
	...
	async execute(params: ReadCommandOutputParams, task: ITaskModel, callbacks: ToolCallbacks): Promise<void> {
		...
		if (!artifact_id) {
			...
			const errorMsg = await sayAndCreateMissingParamError(task.taskId, "read_command_output", "artifact_id")
