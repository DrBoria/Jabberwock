...
/**
 * Helper to access terminalProcess on the Task instance at runtime.
 * Uses intersection type assertion to avoid `as unknown` casts.
 */
function getTaskWithTerminal(
	task: ITaskModel,
...
import { ask, supersedePendingAsk } from "../task/notifications/actions/ask"
...
export class ExecuteCommandTool extends BaseTool<"execute_command"> {
	...
	async execute(params: ExecuteCommandParams, task: ITaskModel, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const terminalShellIntegrationDisabled = true
