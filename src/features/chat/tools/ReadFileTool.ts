...
import { DEFAULT_LINE_LIMIT } from "../../settings/context/tools/native-tools/read_file"
...
export class ReadFileTool extends BaseTool<"read_file"> {
	...
	private async executeNew(params: ReadFileParams, task: ITaskModel, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const maxImageFileSize = DEFAULT_MAX_IMAGE_FILE_SIZE_MB
			const maxTotalImageSize = DEFAULT_MAX_TOTAL_IMAGE_SIZE_MB
				...
				try {
					...
					if (isBinary) {
						await this.handleBinaryFile(
							...
							fullPath,
							supportsImages,
							maxImageFileSize as number,
							maxTotalImageSize as number,
							imageMemoryTracker,
							updateFileResult,
						)
						continue
	...
	private async requestApproval(
		...
		if (filesToApprove.length > 1) {
			...
			const completeMessage = JSON.stringify({ tool: "readFile", batchFiles } satisfies SayToolData)
	 ...
	 */
	private buildAndPushResult(task: ITaskModel, fileResults: FileResult[], pushToolResult: PushToolResult): void {
		...
		} else if (task._state.didRejectTool) {
	...
	getReadFileToolDescription(blockName: string, second: unknown): string {
		...
		if (
			...
			"path" in second &&
			typeof (second as { [key: string]: unknown }).path === "string"
		) {
			return `[${blockName} for '${(second as { [key: string]: unknown }).path}']`
		}

		const blockParams = second as { [key: string]: unknown }
		if (blockParams?.path) {
