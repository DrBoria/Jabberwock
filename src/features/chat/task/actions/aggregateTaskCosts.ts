/**
 * Minimal interface for the history object used by aggregateTaskCostsRecursive.
 * Only requires the fields that this function actually accesses.
 */
...
export async function aggregateTaskCostsRecursive(
	taskId: string,
	getTaskHistory: (id: string) => Promise<TaskCostHistory | undefined>,
	visited: Set<string> = new Set(),
): Promise<AggregatedCosts> {
	// Prevent infinite loops
	if (visited.has(taskId)) {
		console.warn(`[jabberwock] [aggregateTaskCostsRecursive] Circular reference detected: ${taskId}`)
	...
	if (!history) {
		console.warn(`[jabberwock] [aggregateTaskCostsRecursive] Task ${taskId} not found`)
		return { ownCost: 0, childrenCost: 0, totalCost: 0 }
	}
