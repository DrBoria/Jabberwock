...
import { ApiMessage } from "../../messages/actions/saveApiConversation"
...
export type SummarizeResponse = {
	...
	condenseId?: string // The unique ID of the created Summary message, for linking to condense_context message
}
...
${commandBlocks}
	...
	// Note: generateFoldedFileContext was stubbed out — the implementation file never existed
	//       and the feature was never functional. The non-critical error handling is preserved
	//       as a no-op to avoid breaking the call site.
	if (filesReadByJabberwock && filesReadByJabberwock.length > 0 && cwd) {
		// Folded file context generation is currently disabled.
		// The previous implementation referenced a nonexistent module.
		// See plans/architectural-restructure-v2.md Phase 1C for migration context.
	}
