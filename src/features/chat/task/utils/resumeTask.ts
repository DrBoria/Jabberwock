...
// initiateTaskLoop is a private method on Task; accessed via TaskDelegate
...
export async function resumeTaskFromHistory(task: Task): Promise<void> {
	try {
		...
		const lastRelevantMessageIndex = findLastIndex(
			...
			(m: Record<string, unknown>) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"),
		)
		...
		const lastApiReqStartedIndex = findLastIndex(
			modifiedClineMessages,
			(m: Record<string, unknown>) => m.type === "say" && m.say === "api_req_started",
		)
		...
		const lastClineMessage = task.clineMessages
			.slice()
			.reverse()
			.find((m: Record<string, unknown>) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // Could be multiple resume tasks.
		...
		if (existingApiConversationHistory.length > 0) {
			...
			if ((lastMessage as { isSummary?: boolean }).isSummary) {
				handlerKey = "summary"
		...
		await task.initiateTaskLoop(newUserContent)
	} catch (error) {
