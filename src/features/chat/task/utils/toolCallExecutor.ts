...
export async function waitForToolExecutionAndPrepareNextContent(
	...
	while (!waitResolved && !task.abort) {
		if (task.userMessageContentReady) {
			waitResolved = true
			break
		}
		...
		if (elapsed >= 60_000) {
			...
			const hasPendingInteractiveApp =
				lastMsg?.type === "ask" && lastMsg?.ask === "interactive_app" && task.getAskResponse() === undefined
			...
			console.error(
				`[Task#${task.taskId}] pWaitFor(userMessageContentReady) timed out after ${elapsed}ms. ` +
					`Current Index: ${String(task.currentStreamingContentIndex)}, ` +
					`Blocks: ${String(assistantMsgContent.length)}, ` +
					`Locked: ${String(task.presentAssistantMessageLocked)}, ` +
					`didAlreadyUseTool: ${String(task.didAlreadyUseTool)}`,
			)
			...
			if (task.presentAssistantMessageLocked && userMessageContent.length === 0) {
				...
				task.currentStreamingContentIndex = assistantMsgContent.length
			}
	...
	if (!didToolUse) {
		...
		task.consecutiveNoToolUseCount = (task.consecutiveNoToolUseCount as number) + 1

		// Only show error and count toward mistake limit after 2 consecutive failures
		if ((task.consecutiveNoToolUseCount as number) >= 2) {
			await task.say("error", "MODEL_NO_TOOLS_USED")
			// Only count toward mistake limit after second consecutive failure
			task.consecutiveMistakeCount = (task.consecutiveMistakeCount as number) + 1
		}
	...
	const userMsgContent = task.userMessageContent
	if (userMsgContent.length > 0 || task.isPaused) {
