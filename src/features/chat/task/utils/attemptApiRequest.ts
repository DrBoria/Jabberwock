...
// ClineProvider was renamed to EventBridge; no longer imported here
 ...
 */
function getCurrentProfileId(state: Record<string, unknown> | undefined): string {
	return (
		(
			(state as Record<string, unknown>)?.listApiConfigMeta as Array<{ name: string; id?: string }> | undefined
		)?.find((profile) => profile.name === (state as Record<string, unknown>)?.currentApiConfigName)?.id ?? "default"
	)
}
...
export async function handleContextWindowExceededError(task: Task): Promise<void> {
	...
	if (provider) {
		const toolsResult = await buildNativeToolsArrayWithRestrictions({
			...
			cwd: task.cwd,
			mode,
			customModes: state?.customModes as ModeConfig[] | undefined,
...
export async function backoffAndAnnounce(
	task: Task,
	retryAttempt: number,
	error: { status?: number; message?: string; errorDetails?: Array<Record<string, unknown>> },
): Promise<void> {
	try {
		const state = await task.providerRef.deref()?.getState()
		...
		if (error?.status === 429) {
			const retryInfo = (error?.errorDetails as Array<Record<string, unknown>> | undefined)?.find(
...
export async function* attemptApiRequest(
	...
	if (providerInstance && providerInstance.chatStore) {
		...
		while (currentNodeId) {
			...
			const nodeMessages = node.messages.map((m: { content: ApiMessage }) => m.content)
	...
	const approvalResult = await task.autoApprovalHandler.checkAutoApprovalLimits(
		state as GlobalState | undefined,
		task.combineMessages(task.clineMessages.slice(1)),
		async (type: unknown, data: unknown) => task.ask(type as never, data as never),
	)
	...
	try {
		...
		let firstChunkTimeoutRef: ReturnType<typeof setTimeout> | undefined
		const timeoutPromise = new Promise<never>((_, reject) => {
			firstChunkTimeoutRef = setTimeout(
		...
		if (firstChunkTimeoutRef) {
			clearTimeout(firstChunkTimeoutRef)
		}
			...
			if (retryAttempt >= MAX_AUTO_RETRY_ATTEMPTS) {
				console.error(
					`[Task#${task.taskId}] Auto-retry limit reached (${MAX_AUTO_RETRY_ATTEMPTS}). ` +
						`Giving up on request. Error: ${((error as Record<string, unknown> | undefined)?.message as string | undefined) ?? String(error)}`,
				)
				throw new Error(
					`[Task#attemptApiRequest] task ${task.taskId}.${task.instanceId} ` +
						`auto-retry limit reached after ${MAX_AUTO_RETRY_ATTEMPTS} attempts. ` +
						`Last error: ${((error as Record<string, unknown> | undefined)?.message as string | undefined) ?? String(error)}`,
				)
			}

			// Apply shared exponential backoff and countdown UX
			await backoffAndAnnounce(
				task,
				retryAttempt,
				error as { status?: number; message?: string; errorDetails?: Array<Record<string, unknown>> },
			)
	...
	while (true) {
		let inactivityTimeoutRef: ReturnType<typeof setTimeout> | undefined
		const timeoutPromise = new Promise<never>((_, reject) => {
			inactivityTimeoutRef = setTimeout(
