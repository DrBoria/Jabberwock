		...
		reasoning: async (chunk) => {
			...
			if (provider && provider.chatStore) {
				...
				if (node) {
					...
					node.updateApiMessage(task.instanceId, {
						role: "assistant",
						content: [{ type: "reasoning", text: state.reasoningMessage }, ...assistantMsgContent],
					})
				}
			}
		},
		...
		tool_call_partial: async (chunk) => {
			...
			for (const event of events) {
				if (event.type === "tool_call_start") {
					...
					const streamingToolCallIndices = task.streamingToolCallIndices
				...
				if (node) {
					node.updateApiMessage(task.instanceId, {
						...
						content: (state.reasoningMessage
							? [{ type: "reasoning" as const, text: state.reasoningMessage }]
							: ([] as AssistantMessageContent[])
						).concat(assistantMsgContent),
					})
				}
			}
		},
