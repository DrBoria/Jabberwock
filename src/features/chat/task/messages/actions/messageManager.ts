...
import { ApiMessage } from "./saveApiConversation"
import { cleanupAfterTruncation } from "../../condense/handlers/on-context-condense"
import { OutputInterceptor } from "../../../../../integrations/terminal/OutputInterceptor"
import { getTaskDirectoryPath } from "../../../../../utils/storage"
import { overwriteNotifications } from "../../notifications/actions/overwriteNotifications"
import { overwriteApiConversationHistory } from "./saveApiConversation"
...
export class MessageManager {
	 ...
	 * @throws Error if timestamp not found in messages
	 */
	async rewindToTimestamp(ts: number, options: RewindOptions = {}): Promise<void> {
		...
		// Find the index in messages
		const msgIndex = this.task.messages.findIndex((m) => m.ts === ts)
		if (msgIndex === -1) {
	...
	/**
	 * Rewind conversation to a specific index in messages.
	...
	async rewindToIndex(toIndex: number, options: RewindOptions = {}): Promise<void> {
		const cutoffTs = this.task.messages[toIndex]?.ts ?? Date.now()
	 ...
	 * - condense_context (message) ↔ Summary (apiMessage)
	 * - sliding_window_truncation (message) ↔ Truncation marker (apiMessage)
	 */
	private collectRemovedContextEventIds(fromIndex: number): ContextEventIds {
		...
		for (let i = fromIndex; i < this.task.messages.length; i++) {
			const msg = this.task.messages[i]
	...
	private async truncateClineMessages(toIndex: number): Promise<void> {
		await overwriteNotifications(this.task.taskId, this.task.messages.slice(0, toIndex))
	}
	 ...
	 * Due to async execution during streaming, message timestamps may not
	 * perfectly align with API message timestamps. Specifically, a "user_feedback"
	 * message can have a timestamp BEFORE the assistant API message that
	...
	private async truncateApiHistoryWithCleanup(
		...
		if (!hasExactMatch && hasMessageBeforeCutoff) {
			...
			// condition where the message timestamp is earlier than any API message
		...
		if (!skipCleanup) {
			...
			// Collect timestamps from remaining messages
			for (const msg of this.task.messages) {
				if (msg.ts) {
			...
			this.cleanupOrphanedArtifacts(validIds).catch((error) => {
				console.error(
					"[jabberwock] [MessageManager] Error cleaning up orphaned command output artifacts:",
					error,
				)
			})
		}
		...
		if (historyChanged) {
			await overwriteApiConversationHistory(this.task, apiHistory)
		}
	}
