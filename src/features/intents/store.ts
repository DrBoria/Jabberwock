...
/**
 * Base payload type for intents.
 *
 * Each intent type has specific payload fields — define a discriminated union
 * keyed by `IntentType` for full type safety. This base type provides a
...
export type IIntentPayload =
	...
	| { mode?: string }
	| { promptMode?: string; customPrompt?: { [key: string]: unknown } }
	| { systemPromptTemplateKey?: string; systemPromptTemplate?: string }
	| { [key: string]: unknown }

/**
 * A single intent in the MST store.
 *
 * Intents are reactive entities — when one is created with Queued status,
 * the IntentBus picks it up, sets it to Processing, runs the handler,
 * and marks it Success or Failed.
 */
