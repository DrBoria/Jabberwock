...
export class ProviderSettingsManager {
	/** @deprecated Old prefix used by "roo-cline" — kept for migration from legacy keys */
	private static readonly LEGACY_SCOPE_PREFIX = "roo_cline_config_"
	private static readonly SCOPE_PREFIX = "jabberwock_persistent_config_"
	...
	private async load(): Promise<ProviderProfiles> {
		try {
			...
			// Migrate from legacy key (roo_cline_config_) to new key (jabberwock_persistent_config_)
			if (!content) {
				const legacyKey = `${ProviderSettingsManager.LEGACY_SCOPE_PREFIX}api_config`
				const legacyContent = await this.context.secrets.get(legacyKey)
