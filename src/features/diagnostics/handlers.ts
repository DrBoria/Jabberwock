...
export async function generateErrorDiagnostics(params: GenerateDiagnosticsParams): Promise<GenerateDiagnosticsResult> {
	...
	try {
		...
		const tempFileName = `jabberwock-diagnostics-${taskId.slice(0, 8)}-${timestamp}.json`
...
export const handlerMap: Record<string, HandlerFn> = {
	clearDiagnostics: async (provider, _message) => {
		const { diagnosticsManager } = await import("@jabberwock/devtool")
		diagnosticsManager.clear()
		const { postStateToWebview } = await import("../../features/foundation/window-manager/store")
