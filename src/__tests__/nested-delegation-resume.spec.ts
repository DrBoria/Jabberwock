	...
	it("C completes → reopens B; then B completes → reopens A; emits correct events; no resume_task asks", async () => {
		...
		const provider = {
			...
			reopenParentFromDelegation: vi.fn(async (params: any) => {
				return await (EventBridge.prototype as any).reopenParentFromDelegation.call(provider, params)
			}),
		...
		expect(eventNamesAfterC).toContain(JabberwockEventName.TaskDelegationCompleted)
		expect(eventNamesAfterC).toContain(JabberwockEventName.TaskDelegationResumed)
		...
		const completedEvents = emitSpy.mock.calls.filter(
			(c: any[]) => c[0] === JabberwockEventName.TaskDelegationCompleted,
		)
		const resumedEvents = emitSpy.mock.calls.filter(
			(c: any[]) => c[0] === JabberwockEventName.TaskDelegationResumed,
		)
