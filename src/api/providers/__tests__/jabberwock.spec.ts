// npx vitest run api/providers/__tests__/jabberwock.spec.ts

import { Anthropic } from "@anthropic-ai/sdk"
import { rooDefaultModelId } from "@jabberwock/types"
...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key: string) => {
		if (key === "common:errors.jabberwock.authenticationRequired") {
			return "Authentication required for Jabberwock Cloud"
		}
		return key
	}),
}))
...
describe("RooHandler", () => {
			...
			expect(mockCreate).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					headers: expect.objectContaining({
						"X-Jabberwock-App-Version": expect.any(String),
					}),
				}),
			)
		})
	...
	describe("completePrompt", () => {
		...
		it("should handle API errors", async () => {
			...
			await expect(handler.completePrompt("Test prompt")).rejects.toThrow(
				"Jabberwock Cloud completion error: API Error",
			)
		})
	...
	describe("getModel", () => {
		...
		it("should handle any model ID since models are loaded dynamically", () => {
			...
			const testModelIds = ["xai/grok-code-fast-1", "jabberwock/sonic", "deepseek/deepseek-chat-v3.1"]
	...
	describe("temperature and model configuration", () => {
		...
		it("should use correct API endpoint", () => {
			// The base URL should be set to Jabberwock's API endpoint
			// We can't directly test the OpenAI client configuration, but we can verify the handler initializes
			handler = new RooHandler(mockOptions)
			expect(handler).toBeInstanceOf(RooHandler)
			// The handler should work with the Jabberwock API endpoint
		})
	})
