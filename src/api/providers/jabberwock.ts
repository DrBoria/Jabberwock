...
export class RooHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	protected override createStream(
		...
		// Get Jabberwock-specific reasoning parameters
	...
	override async *createMessage(
		...
		try {
			...
			if (lastUsage) {
				...
				const apiProtocol = getApiProtocol("jabberwock", modelId)
	...
	/**
	 * Generate an image using Jabberwock Cloud's image generation API
